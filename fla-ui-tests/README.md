...
| `.\run-reload-drug-row.ps1 -Probe -Seed` | nạp 診療入力 → `ModSave.GetTrnRs` **dựng lại** ô 療法・処置 của dòng 薬剤 từ master + `freewd` (bỏ qua `dsp_trt` đã lưu) rồi khoá ô | `Tests/DrugRowReload/` | ⚠️ **CÓ — nặng nhất bộ**: chèn dòng vào `TRNTRN` (処置行 thật). Chỉ chèn `DISP_NO` riêng 9201/9202 rồi xoá; đo đường LOAD nên dữ liệu buộc phải có sẵn trước khi mở màn |
| `.\run-resolve-drug-path-b.ps1 -Probe -Seed` | gõ mã thuốc KHÔNG có `mst_drug_rx` → `editDrugName` **path B** (tên từ `getMstTrtDataYaku`, 用法 từ `MST_MED`) | `Tests/DrugPathB/` | ⚠️ **CÓ** — giấu tạm dòng `MST_DRUG_RX` (bảng 処置変換 **dùng chung**); dev có 0/63 mã rơi vào path B nên không seed thì nhánh không bao giờ chạy |
