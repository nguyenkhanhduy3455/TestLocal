/**
 * 診療入力 F9 登録 — side-effect của `modSave.SaveData`, NHÓM P0.
 *
 * MỘT CHỨC NĂNG = MỘT FILE. Hai spec cũ cùng đo side-effect của MỘT lần bấm F9,
 * chỉ khác lô thi công: khối 1 gom các gap chưa port, khối 2 là lô 5 (`LetHokan`
 * + `Let_BNOW` — 修復物データ作成 & 未装着一覧), phần duy nhất của lô đó mà integration
 * test không che được vì phải seed `mst_trt` (bảng 74 cột NOT NULL).
 *
 * ĐẶC TÍNH KIỂM THỬ giữ nguyên: mọi assert bám THEO WINFORM, file được viết để
 * **ĐỎ TRƯỚC KHI SỬA** — mỗi TC đỏ là một gap THẬT, không phải test viết sai;
 * xanh dần theo từng lô thi công.
 *
...
// ═══ nguyên văn từ p0-oral-chart-let-bnow.spec.ts (đã gộp vào file này) ════════════════════════════════
test.describe('lô 5: LetHokan / Let_BNOW (口腔内チャート)', () => {
 ...
 * ═════════════════════════════════════════════════════════════════════════════
 *   cd /Users/thinhnn/Documents/GitHub/TestLocalApp/TestLocal/web-tenant-tests
 *   TEST_DB=1 TEST_ALLOW_SAVE=1 npx playwright test tests/save-f9/save-f9/p0-save-side-effects.spec.ts
 *
