...
interface TabCfg {
  ...
  virtual?: boolean
  /**
   * Mọi cột đều là px cố định (chỉ 個別). Ba tab kia kết thúc bằng `1fr` nên tổng
   * track LUÔN đúng bằng khung — chúng không thể tràn, `1fr` tự co khi có thanh
   * cuộn — nên phép trừ hao thanh cuộn chỉ áp cho tab này.
   */
  fixedTracks?: boolean
}
...
test.describe('SidePanel — 見出しクリックの並べ替え (frm203002 4タブ)', () => {
  ...
  async function expectNoHorizontalScroll(cfg: TabCfg) {
    const m = await headerCell(cfg.headers[0] ?? '').evaluate((el) => {
      ...
      return {
        tracks: Math.round(cells.reduce((w, c) => w + c.getBoundingClientRect().width, 0)),
    ...
    expect(
      m.scroll,
      `${cfg.tab}: lưới rộng hơn vùng cuộn ${m.scroll - m.client}px → panel cuộn ngang.`,
    ).toBeLessThanOrEqual(m.client + 1)

    if (cfg.fixedTracks) {
      expect(
        m.tracks,
        `${cfg.tab}: tổng bề rộng cột ${m.tracks}px không lọt ${m.client}px của vùng cuộn khi ` +
          `trừ ${SCROLLBAR_RESERVE}px thanh cuộn dọc → máy bật "Show scroll bars: Always" sẽ cuộn ngang.`,
