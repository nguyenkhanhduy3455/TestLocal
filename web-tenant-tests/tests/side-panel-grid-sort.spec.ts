 ...
 *      · Header 病検 `grid-cols-[44px_270px_1fr]` (:1002), ガイド `[46px_1fr]`
 *        (:1089), パック `[42px_1fr]` (:1135), 個別
 *        `[160px_48px_66px_48px_60px_48px]` (KOBE_GRID_CLASS :53).
...
/** Bề rộng thanh cuộn dọc phải chừa cho máy bật "Show scroll bars: Always". */
...
const TABS: Record<TabName, TabCfg> = {
  ...
  個別: {
    tab: "個別",
    rowSel:
      'div[class*="grid-cols-[160px_48px_66px_48px_60px_48px]"][class*="cursor-pointer"]',
...
test.describe("SidePanel — 見出しクリックの並べ替え (frm203002 4タブ)", () => {
  ...
  async function expectHeadersSingleLine(cfg: TabCfg, when: string) {
    for (const label of cfg.headers) {
      ...
      await expect(
        cell,
        `${cfg.tab}: không thấy tiêu đề 「${label}」`,
      ).toBeVisible();
  ...
  /**
   * Lưới của tab phải vừa vùng cuộn, VÀ nền dòng tiêu đề phải phủ hết các cột.
   *
   * Hai lỗi thật đã gặp:
   *   1. Nới cột cho vừa icon sort mà quên tổng bề rộng → cuộn ngang, vừa bấm
   *      cột phải là cột 処置名称 bị cắt mất chữ đầu.
   *   2. Khi lưới rộng hơn khung, hộp dòng tiêu đề chỉ rộng bằng KHUNG nên nền
   *      `bg-muted` hụt ở đuôi: cuộn dọc sẽ thấy các dòng chạy XUYÊN QUA phần
   *      tiêu đề của cột cuối (trông như header trong suốt).
   *
   * Phải trừ hao SCROLLBAR_RESERVE: Chromium ở đây dùng thanh cuộn overlay (0px)
   * nên clientWidth là bề rộng đầy đủ, còn máy đặt 「Show scroll bars: Always」
   * mất thêm ~15-17px. Không trừ hao thì test xanh ở đây mà vẫn vỡ trên máy
   * thật — đúng những gì đã xảy ra.
   */
  async function expectNoHorizontalScroll(cfg: TabCfg) {
    const m = await headerCell(cfg.headers[0] ?? "").evaluate((el) => {
      ...
      return {
        tracks: Math.round(
          cells.reduce((w, c) => w + c.getBoundingClientRect().width, 0),
        ),
        client: scroller.clientWidth,
        rowRight: Math.round(row.getBoundingClientRect().right),
        lastRight: Math.round((last ?? el).getBoundingClientRect().right),
      };
    });

    expect(
      m.tracks,
      `${cfg.tab}: tổng bề rộng cột ${m.tracks}px không lọt ${m.client}px của vùng cuộn khi trừ ` +
        `${SCROLLBAR_RESERVE}px thanh cuộn dọc → máy bật "Show scroll bars: Always" sẽ cuộn ngang.`,
    ).toBeLessThanOrEqual(m.client - SCROLLBAR_RESERVE);

    expect(
      m.rowRight,
      `${cfg.tab}: nền dòng tiêu đề hụt ${m.lastRight - m.rowRight}px so với cột cuối → ` +
        `cuộn dọc sẽ thấy dòng chạy xuyên qua tiêu đề cột cuối (thiếu w-max min-w-full).`,
    ).toBeGreaterThanOrEqual(m.lastRight - 1);
  }
  ...
  async function openTab(cfg: TabCfg): Promise<number> {
    ...
    await expect(headerCell(cfg.headers[0] ?? "")).toBeVisible({
      timeout: 60000,
    });
    await step();
    ...
    await expect(
      rows.first().or(page.getByText(/未登録|該当なし/)),
    ).toBeVisible({
      timeout: 60000,
    });
  ...
  async function clickHeader(
    ...
    await expect(
      cell,
      `「${label}」: aria-sort không chuyển sang ${expected}`,
    ).toHaveAttribute("aria-sort", expected);
    await step();
  }
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const url = TRT_DT
      ? `/treatments/${PAT_NO}?trtDt=${TRT_DT}`
      : `/treatments/${PAT_NO}`;
  ...
  test("TC-1 病検 — 3 cột đều sortable, mở tab là CHƯA sort (rule 1)", async () => {
    ...
    if (n < MIN_ROWS) {
      console.log(
        `TC-1 病検: chỉ ${n} dòng (<${MIN_ROWS}) → BỎ QUA nhóm 病検. Đổi TEST_PAT_NO.`,
      );
      test.skip();
    }
    for (const label of cfg.headers) {
  ...
  test("TC-2 病検 — sort 病名: không mất dòng, № đi theo dòng, không mở dialog", async () => {
    ...
    await expect(
      headerCell("病名"),
      "病検: click header lại nhảy sang tab khác",
    ).toBeVisible();
  ...
  test('TC-3 病検 — bấm lại 病名: đảo chiều, không quay về "chưa sort" (rule 3)', async () => {
    ...
    expect(ranks, "病検: desc chứa giá trị không có trong asc").not.toContain(
      -1,
    );
    expect(
      ranks,
  ...
  test("TC-5 病検 — sort xong con trỏ vẫn ở đúng dòng cũ, ô 選択№ bám theo", async () => {
    ...
    const highlighted = async () =>
      rows.evaluateAll((els) => {
        ...
        return {
          idx: i,
          text: (el.children[2] as HTMLElement).innerText.trim(),
        };
      });
    ...
    expect(after?.text, "病検: sort xong con trỏ nhảy sang dòng KHÁC").toBe(
      before?.text,
    );
    await expect(
      noInput(),
  ...
  test("TC-6 ガイド — sort 名称 + № đi theo dòng + không mở dialog chọn ガイド", async () => {
    ...
    await step();
    const rows = rowsOf(cfg);
    await expect(rows.first().or(page.getByText("未登録"))).toBeVisible({
      timeout: 60000,
    });
    const before = await readRows(cfg, 1);
    if (before.length < MIN_ROWS) {
      console.log(
        `TC-6 ガイド: chỉ ${before.length} dòng → BỎ QUA nhóm ガイド.`,
      );
      test.skip();
    }
    ...
    expect(
      [...asc.map((r) => r.text)].sort(),
      "ガイド: sort làm mất/nhân bản dòng",
    ).toEqual([...before.map((r) => r.text)].sort());
    const nameByNo = new Map(before.map((r) => [r.no, r.text]));
    for (const r of asc) {
      expect(
  ...
  test("TC-7 パック — sort 名称 + № đi theo dòng + không mở パック処置選択", async () => {
    ...
    expect(
      [...asc.map((r) => r.text)].sort(),
      "パック: sort làm mất/nhân bản dòng",
    ).toEqual([...before.map((r) => r.text)].sort());
    const nameByNo = new Map(before.map((r) => [r.no, r.text]));
    for (const r of asc) {
      expect(
  ...
  test("TC-8 個別 — sort cột số (コード) tăng/giảm đúng, header 6 cột không vỡ", async () => {
    ...
    const codes = async () =>
      rowsOf(cfg).evaluateAll((els) =>
        els.map((el) =>
          Number((el.children[4] as HTMLElement).innerText.trim()),
        ),
      );
    ...
    expect(
      asc.length,
      "個別: sort xong không còn dòng nào render",
    ).toBeGreaterThan(0);
    expect(asc, "個別: コード ▲ không tăng dần").toEqual(
      [...asc].sort((a, b) => a - b),
    );
    ...
    expect(desc, "個別: コード ▼ không giảm dần").toEqual(
      [...desc].sort((a, b) => b - a),
    );
    expect(
      Number(desc[0]),
      "個別: ▼ mà đỉnh bảng không lớn hơn đỉnh của ▲",
    ).toBeGreaterThan(Number(asc[0]));
    await expectHeadersSingleLine(cfg, "đang sort コード ▼");
    ...
    await expectNoDialogOpened(cfg);
    console.log(
      `TC-8 個別: コード ▲ top=[${asc.slice(0, 5).join(", ")}] ▼ top=[${desc.slice(0, 5).join(", ")}]`,
    );
  });
});
