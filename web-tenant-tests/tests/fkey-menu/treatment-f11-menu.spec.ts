/**
 * 診療入力 — menu 「選択」 bật bằng F11 (RowContextMenu ⇐ WinForm frm203002
 * `contextMenuStripSentaku`).
 *
 * MỘT MENU = MỘT FILE. Trước đây menu này bị chẻ làm hai spec: một đo VIỆC HIỆN
 * RA (menu + submenu 9 オプション không bị cắt ở mép màn), một đo CÁC MỤC làm gì khi
 * bấm. Cùng một menu, cùng `row-context-menu.tsx` — sửa menu là phải mở cả hai.
 *
 * Hai khối dưới đây giữ NGUYÊN VĂN nội dung hai file cũ.
 */

import { Page, Locator, type Route } from '@playwright/test'
...
// ═══ nguyên văn từ treatment-f11-fkey-button-show-modal.spec.ts (đã gộp vào file này) ══════════════════════
test.describe('hiển thị: 9 オプション submenu không bị cut off', () => {
...
// ═══ nguyên văn từ treatment-f11-menu-ported-actions.spec.ts (đã gộp vào file này) ═════════════════════════
test.describe('hành vi các mục vừa port', () => {
 ...
 *
 * File này KHÁC `fkey-menu/treatment-f11-menu.spec.ts`: file kia lo việc
 ...
 *
 * ─── Cách chạy ───────────────────────────────────────────────────────────────
 *   TEST_DB=1 npx playwright test tests/fkey-menu/treatment-f11-menu.spec.ts --retries=0
 *
